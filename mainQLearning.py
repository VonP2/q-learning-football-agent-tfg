...
from monitor.trainingMonitor import TrainingMonitor
...
class FootballGameLQ:
    ...
    def train_ai(self, agentL, agentR, done):
        ...
        if agentL.done() or agentR.done():
            ...
            if self.num_resets %100 == 0:
                ...
                monitor.update_plot(self.num_resets, agentL.rewards, agentR.rewards, mean, meanL, meanR)

                if self.num_resets%1000==0:
                    monitor.save_plot("graphs/grafica.png")
    ...
    def play_ai_single_agent(self, agentL, agentR):
        ...
        if agentL.done() or agentR.done():
            ...
            monitor.update_plot(self.num_resets, agentL.rewards, agentR.rewards, mean, meanL, meanR)
            monitor.save_plot("graphs/graficaResultado.png")
    ...
    def test_agent(self, agentL, agentR,steps):
        agentL.epsilon = 0.0
        agentR.epsilon = 0.0
        ...
        if not agentL.done() or not agentR.done():
        ...
        if agentL.done() or agentR.done():
            ...
            print(f"Mean reward Left Agent: {meanL}")
            ...
            total_goals_L = self.game.left_team_score
            total_goals_R = self.game.right_team_score
            print(f"Goal scoring rate Left Agent: {total_goals_L/self.num_resets:.2%}")
