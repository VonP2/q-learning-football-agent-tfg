...
class BallUser(pygame.sprite.Sprite):
	"""
	CONSTRUCTOR
	"""
	...
	def collision(self,direction):
			...
			if direction == 'horizontal':
				for sprite in collision_sprites:
					...
					if self.rect.right >= sprite.rect.left and self.old_rect.right <= sprite.old_rect.left:
						...
						if isinstance(sprite, (Player, Playeruser)): 
		...
		if self.x_speed != 0:
			if self.rect.left < 0:
				...
				self.pos.x = self.rect.x
				self.x_speed *= -1
			if self.rect.right > Gameuser.SCREEN_WIDTH:
				self.rect.right = Gameuser.SCREEN_WIDTH
				self.pos.x = self.rect.x
				self.x_speed *= -1
		...
		if self.y_speed != 0:
			if self.rect.top < 0:
				...
				self.pos.y = self.rect.y
				self.y_speed *= -1
			if self.rect.bottom > Gameuser.SCREEN_HEIGHT:
				self.rect.bottom = Gameuser.SCREEN_HEIGHT
				self.pos.y = self.rect.y
				self.y_speed *= -1
